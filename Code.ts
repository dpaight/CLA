...
function importXLS() {
    var folderID = "1nZmeys-WvSux7r2w87eTKzmgaZMpMnTq"; // Added // Please set the folder ID of "FolderB".
    var folder = DriveApp.getFolderById(folderID);
    var files = DriveApp.getFolderById(folderID).getFiles();
    var filesFound = [];
    while (files.hasNext() && filesFound.length < 2) {
      var xFile = files.next();
      var name = xFile.getName();
      if (name.indexOf('PrintQueryToExcel_latest_K5.xlsx') > -1 || name.indexOf('PrintQueryToExcel_latest_612.xlsx') > -1) {
        var ID = xFile.getId();
    ...
    for (let i = 0; i < filesFound.length; i++) {
      var converted = DriveApp.getFileById(filesFound[i]);
      var convertedSS = SpreadsheetApp.openById(filesFound[i]);
      var newDataPart = convertedSS.getSheetByName('Sheet1').getDataRange().getValues();
      var headings = newDataPart.shift();
      newData = newData.concat(newDataPart);
    }
    newData.unshift(headings);
    ...
    var headersAndFormulas = [[
      '=ArrayFormula(iferror(vlookup($M1:$M, teacherCodes!$A$1:$H, 5,false),if(row($M1:$M) = 1, "teachEmail","")))	',
      '=ArrayFormula(iferror(vlookup($M1:$M,{teacherCodes!$A$1:$I }, 2,false),if(row($M$1:$M) = 1,"teachName","")))	',
      '=ArrayFormula(iferror(vlookup($M1:$M, teacherCodes!$A$1:$H, 6,false),if(row($M1:$M) = 1, "teachMeet","")))',
...
function importCsv(fName) {
    if (fName === void 0) { fName = "services"; }
    var data = parseCSV(fName, "1gPgB58lTcUJNX39_DklgwJBW-zH3KeDl");
...
//     var csvFile = range.getValues().join("\n");

//     var csvBlob = Utilities.newBlob(csvFile, 'text/csv', 'filename.csv');

//     // Create a folder to store the CSV files if it does not exist yet
...
//     // Save the CSV file in the newly created folder
//     folder.createFile(csvBlob);

//     Logger.log("Excel file successfully converted to CSV");
// }
function updateRoster() {
    // 
    // get seis data
    // get seis data
    // this gets the aeries
    getExcelData();
    ...
    // seis csv report
    var sVals = parseCSV("roster_seis", "1gPgB58lTcUJNX39_DklgwJBW-zH3KeDl");
    ...
    var sortThis = ss.getSheetByName('roster');
    sortThis.sort(1);
    ...
    var ss2 = allPupilsSheet();
    pSheet = ss.getSheetByName("allPupilsFromAeriesModNames");
    ...
    for (var j = 1; j < rVals.length; j++) {
        ...
        var mySeis_id = rEl[rInd["seis_id"]];
        if (sMatch[r_nmjdob] === undefined && rEl[rInd["seis_id"]].toString()[0] !== "u") {
            // if (sMatch[r_nmjdob] === undefined && rEl[rInd["seis_id"]] !== "u") {
            // record is not in the seis file; assume deleted, unless the seis student id indicates 
            // a record that is not YET in seis, but is expected soon
            ...
            for (var u = 0; u < rEl.length; u++) {
                ...
                if (nextField > -1) {
                    ...
                    try {
                        ...
                        Logger.log('values of j, u, rEl: %s, %s, %s; error: %s', j, u, JSON.stringify(rEl), error);
                        if (rEl[0].indexOf('u') !== -1) {
                            break;
                        }
...
function searchFor(searchStrAndFld) {
    var data = ss.getSheetByName('roster')
        .getDataRange()
        .getValues();
    var [searchStr, fldName] = searchStrAndFld;
    var hits = [];
    var id_index = data[0].indexOf("seis_id");
    var fld_index = data[0].indexOf(fldName);
    for (let i = 0; i < data.length; i++) {
        const el = data[i];
        Logger.log('searchStr is %s', searchStr)
        if (searchStr.search(/^['"].*['"]$/) !== -1) {
            searchStr = new RegExp(searchStr);
        }
        if (el[fld_index].toString().search(searchStr) !== -1) {
            let id = el[id_index].toString();
            hits.push(id);
            // hits[id] = el[fld_index]
        }
    }
    if (hits.length === 0) {
        return false;
    } else {
        Logger.log('hits: %s', JSON.stringify(hits));
        return hits;
    }
}